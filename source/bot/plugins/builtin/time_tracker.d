...
class TimeTrackerPlugin : IPlugin
{
            ...
            while (true)
            {
                ...
                foreach (ref multiplier; multipliers)
                {
                    ...
                    if (!multiplier.channel.isLive
                        || multiplier.username.toLower == ignoreUser.toLower)
                        continue;
    ...
    Abort onGive(IBot bot, string channel, scope Command command)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                bot.send(channel, format("@%s gave @%s %d points. Has %d points left!",
    ...
    {
        bot.send(channel,
                "View the current leaderboard on " ~ websiteBase ~ channel[1 .. $] ~ "/points");
        return Abort.yes;
    }
        ...
        if (url == "confirm")
        {
            ...
            else
            {
                try
                {
                    ...
                    try
                    {
                        auto reader = ret.csvReader!(Tuple!(string, long, long, long))(["Username",
                                "Twitch User ID", "Current Points", "All Time Points"]);
                    ...
                    catch (Exception e)
                    {
                        ...
                        bot.send(channel,
                                "Data not in valid format. Can only import revlobot points.");
                    }
                }
                catch (Exception e)
                {
