...
class GamblerPlugin : IPlugin
{
    this()
    {
        ...
        router.on("!gamble :amount", &gamble,
                "rand(0, 100) where, 100 is 10x, 90..99 = 4x, 66..89 = 2x, else lose");
        router.on("!g :amount", &gamble);
        router.on("!flip :amount", &flip, "rand(0, 1) where 0 = lose, 1 = 2x");
        router.on("!f :amount", &flip);
        router.on("!roulette :amount", &roulette,
                "bet will be multiplied by one of these: 0, 0.5, 1, 1.5, 2");
    ...
    Abort gamble(IBot bot, string channel, scope Command command)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                bot.send(channel, format("@%s rolled %d, %s %d points. Has %d points now!",
                        command.raw.sender, roll, amount > 0 ? "won" : "lost",
    ...
    Abort flip(IBot bot, string channel, scope Command command)
    {
        ...
        if (amount <= 0)
            bot.send(channel, "@" ~ command.raw.sender ~ " use `!flip x`, where x is a positive non-zero integer to gamble x points. (You need to own at least x points)");
        else
        {
            long current = command.raw.senderID.pointsFor(channel);
            if (amount > current)
            ...
            else
            {
                ...
                auto newAmount = command.raw.senderID.pointsFor(channel, amount);
                bot.send(channel, format("@%s got %s, %s %d points. Has %d points now!", command.raw.sender,
                        amount > 0 ? "heads" : "tails", amount > 0 ? "won"
    ...
    Abort roulette(IBot bot, string channel, scope Command command)
    {
        ...
        if (amount <= 0)
            bot.send(channel, "@" ~ command.raw.sender ~ " use `!roulette x`, where x is a positive non-zero integer to gamble x points. (You need to own at least x points)");
        else
        {
            long current = command.raw.senderID.pointsFor(channel);
            if (amount > current)
            ...
            else
            {
                ...
                auto newAmount = command.raw.senderID.pointsFor(channel, amount);
                bot.send(channel, format("@%s %s %d points. Has %d points now!",
                        command.raw.sender, amount > 0 ? "won" : "lost", amount.abs, newAmount));
            }
        }
        return Abort.yes;
    }
    ...
    Abort kill(IBot bot, string channel, scope Command command)
    {
        ...
        if (amount <= 0 || !toUser)
            bot.send(channel, "@" ~ command.raw.sender ~ " use `!kill user x`, where x is a positive non-zero integer to gamble x points. (You need to own at least x points), foreach 1000p = 1%");
        else
        {
            if (toUser[0] == '@')
                toUser = toUser[1 .. $];
