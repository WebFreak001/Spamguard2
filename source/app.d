...
shared static this()
{
    ...
    else
    {
        ...
        plugins.add(timeTrackerPlugin = new TimeTrackerPlugin(website,
                info["username"].get!string, true, 1));
...
void userPoints_data(HTTPServerRequest req, HTTPServerResponse res)
{
            ...
            if (propPtr && propPtr.type == Bson.Type.object)
            {
                ...
                if (timePtr && timePtr.type == Bson.Type.long_)
                {
                    ...
                    try
                    {
                        ...
                        if (isChannelLive)
                        {
                            auto mUser = find!"a.username == b"(timeTrackerPlugin.multipliers,
                                    username);
                            if (!mUser.empty)
    ...
    auto users = allUsers.multiSort!("a.multiplier && !b.multiplier",
            "a.points > b.points").take(100);
